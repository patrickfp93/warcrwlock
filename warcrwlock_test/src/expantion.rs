...
// 🦀 Timestamp: 09/10/2023, 15:47:38  🦀

pub mod with_into_impl {
    use std::ops::Range;
    use warcrwlock::warcrwlock;
    pub type NumberCore = _core::Number;
    use std::{
    ...
    impl Number {
        ...
        fn builder(number: usize) -> Self {
            return NumberCore { number }.into();
        }
    }
    impl Number {
        pub fn extract(&self) -> NumberCore {
            self.clone().into()
        }
    }
    impl From<Number> for NumberCore {
        fn from(wrapper: Number) -> Self {
            let guard = wrapper._core.write().unwrap();
            let ptr = &*guard as *const NumberCore;
            let reply = unsafe { std::ptr::read(ptr) };
            drop(guard);
            return reply;
        }
    }
    impl<'a> From<NumberCore> for Number {
        fn from(_core: NumberCore) -> Self {
            return Self {
    ...
    impl PartialEq for Number {
        fn eq(&self, other: &Self) -> bool {
            let ptr_number = self._core.as_ref() as *const RwLock<NumberCore> as usize;
            let other_ptr_number = other._core.as_ref() as *const RwLock<NumberCore>
                as usize;
    ...
    impl Number {
        fn into_vec_wrapper(bases: Vec<NumberCore>) -> Vec<Number> {
            bases.into_iter().map(|b| b.into()).collect()
        }
        fn into_vec_base(multiples: Vec<Number>) -> Vec<NumberCore> {
            multiples.iter().map(|w| (*w).clone().into()).collect()
        }
    }
    pub struct NumberRefLock<'a, T> {
        _guard: RwLockReadGuard<'a, NumberCore>,
        reference: &'a T,
    }
    impl<'a, T> NumberRefLock<'a, T> {
        fn new(ptr: *const T, guard: RwLockReadGuard<'a, NumberCore>) -> Self {
    ...
    pub struct NumberMutLock<'a, T> {
        _guard: RwLockWriteGuard<'a, NumberCore>,
        reference: &'a T,
        reference_mutable: &'a mut T,
    }
    impl<'a, T> NumberMutLock<'a, T> {
        fn new(ptr: *mut T, guard: RwLockWriteGuard<'a, NumberCore>) -> Self {
            let reference = unsafe { &*ptr };
    ...
    mod _into_0 {
        use super::*;
        use super::NumberCore as Number;
        impl Number {
    ...
    impl Number {
        fn new(number: usize) -> Self {
            let reply = NumberCore::new(number).into();
            return reply;
        }
